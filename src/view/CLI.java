...
import model.Point;
import model.node.Building;
import model.node.Canteen;
...
public class CLI {
	...
	private static Point destinationLoc;
	...
	private static boolean isLocationRight(String loc) {
		...
		if (loc.contains("loc-") || loc.contains(",")) {
			retVal = true;
		}
		return retVal;
	}
	...
	private static void choiceMenu(int n) {
		if (n == 1) {
			...
			String[] temp = userLocation.split(",");
			userLoc = new Point(Double.parseDouble(temp[0]), Double.parseDouble(temp[1]));
			printSeparator();
			printMainMenu();
		} else if (n == 2) {
			...
			String[] temp = userLocation.split(",");
			destinationLoc = new Point(Double.parseDouble(temp[0]), Double.parseDouble(temp[1]));
			printSeparator();
			printMainMenu();
		} else if (n == 3) {
			if (!isLocationEntered("user")) {
				...
				Canteen canteen = (Canteen) mc.nearestCanteen(new Node(userLoc));
				String nearestCanteen = canteen.getName();
				String canteenLoc1 = canteen.getLocation().toString();
			...
			if (!isLocationEntered("user")) {
				...
				Building building = (Building) mc.nearestRestRoom(new Node(userLoc));
				Restroom restroom = null;
				for (int i = 0; i < building.getListOfRooms().size(); i++) {
					if (building.getListOfRooms().get(i) instanceof Restroom) {
						restroom = (Restroom) building.getListOfRooms().get(i);
					}
				}

				String nearestRestRoom = restroom.getName();

				System.out.print("|\tThe nearest Restroom is " + nearestRestRoom);
				writeSameString(" ", width - 34 - nearestRestRoom.length());
				System.out.println("|");
			}
			printSeparator();
			printMainMenu();
			...
			if (isLocationEntered("user")) {
				try {
					List<Node> nodes = XMLProcessor.loadFromXml("petaITB.xml");
